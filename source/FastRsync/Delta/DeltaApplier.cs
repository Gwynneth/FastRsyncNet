...
namespace FastRsync.Delta {
    public class DeltaApplier {
        ...
        public async Task ApplyAsync(Stream basisFileStream, IDeltaReader delta, Stream outputStream, CancellationToken ct = default) {
            var buffer = new byte[readBufferSize];

            await delta.ApplyAsync(
                writeData: async (data) => await outputStream.WriteAsync(data, 0, data.Length, ct).ConfigureAwait(false),
                copy: async (startPosition, length) => {
                    basisFileStream.Seek(startPosition, SeekOrigin.Begin);

                    int read;
                    long soFar = 0;
                    while ((read = await basisFileStream.ReadAsync(buffer, 0, (int)Math.Min(length - soFar, buffer.Length), ct).ConfigureAwait(false)) > 0) {
                        if (ct.IsCancellationRequested) return;
                        soFar += read;
                        await outputStream.WriteAsync(buffer, 0, read, ct).ConfigureAwait(false);
                    }
